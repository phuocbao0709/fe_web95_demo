    ...
    <div className='cart-page'>
        ...
        {data.length === 0 && !loading ? (
            <div className='cart-empty'>
                ...
                <p className='cart-empty__text'>Add a few products to bring the checkout panel to life.</p>
            </div>
        ) : (
                ...
                <div className='cart-list'>
                    {loading ? (
                        loadingCart?.map((el,index) => (
                            <div key={el+"Add To Cart Loading"+index} className='cart-card cart-card--loading animate-pulse' />
                        ))
                    ) : (
                        data.map((product,index)=>{
                            ...
                            const imageUrl = getImageUrl(product?.productId?.productImage)
                            const itemTotal = (product?.productId?.sellingPrice || 0) * (product?.quantity || 0)
                            ...
                            return(
                                ...
                                >
                                    <div className='cart-card__media'>
                                        {imageUrl ? <img src={imageUrl} className='cart-card__image' /> : <div className='image-placeholder-text image-placeholder-text--dark image-placeholder-text--small'>No image</div>}
                                    </div>
                                    ...
                                    <div className='cart-card__content'>
                                        <div className='cart-card__header'>
                                            <div className='cart-card__copy'>
                                                ...
                                                <p className='cart-card__meta'>
                                                    {product?.productId?.brandName || "TechPulse"} / Qty {product?.quantity}
                                                </p>
                                            </div>
                                        ...
                                        <div className='cart-card__footer'>
                                            <div className='cart-qty'>
                                                <button className='cart-qty__button' onClick={()=>decraseQty(product?._id,product?.quantity)}>
                                                    <FiMinus />
                                                </button>
                                                <span className='cart-qty__value'>
                                                    {String(product?.quantity).padStart(2,'0')}
                                                </span>
                                                <button className='cart-qty__button' onClick={()=>increaseQty(product?._id,product?.quantity)}>
                                                    <FiPlus />
                                                </button>
                                            </div>
                    ...
                    <div className='cart-summary'>
                        ...
                        <div className='cart-summary__rows'>
                            ...
                            <div className='cart-summary__row'>
                                ...
                                <p className='cart-summary__value'>{displayINRCurrency(totalPrice)}</p>
                            </div>
                            <div className='cart-summary__row'>
                                <p className='cart-summary__label'>Shipping</p>
                                <p className='cart-summary__value'>{shippingCost === 0 ? "FREE" : displayINRCurrency(shippingCost)}</p>
                            </div>
                            <div className='cart-summary__row'>
                                <p className='cart-summary__label'>Tax</p>
                                <p className='cart-summary__value'>{displayINRCurrency(taxAmount)}</p>
                            </div>
                            <div className='cart-summary__divider' />
                            <div className='cart-summary__row cart-summary__row--total'>
                                <p className='cart-summary__total-label'>Total</p>
                                <div>
                                    <p className='cart-summary__total-value'>{displayINRCurrency(grandTotal)}</p>
                                    <p className='cart-summary__hint'>Secure checkout ready</p>
                                </div>
                            </div>
                        </div>

                        <button className='cart-summary__button'>Proceed To Payment</button>
                    </div>
                </aside>
            </div>
        )}
